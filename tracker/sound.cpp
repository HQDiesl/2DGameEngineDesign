...
/*
This class contains the sounds used in the game in a vector.
It also handles starting and stopping certain clips on demand. 
*/
  ...
  channels()
{
  ...
  music = Mix_LoadMUS("sound/Terraria.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load BGM")+Mix_GetError();


  sounds.push_back( Mix_LoadWAV("sound/bow_fire1.wav") );
  sounds.push_back( Mix_LoadWAV("sound/bow_fire2.wav") );
  sounds.push_back( Mix_LoadWAV("sound/Thump.wav") );
  sounds.push_back( Mix_LoadWAV("sound/pop.wav") );
...
void SDLSound::startMusic() {
  Mix_VolumeMusic(SDL_MIX_MAXVOLUME*.75);
